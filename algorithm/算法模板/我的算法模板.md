...
int pos=0;
int max_right=0;
int ans_rad=0;  //最长回文串的半径长度
int ans_pos=0;   //最长回文串的中心轴位置
int rad_right[3000];   //每个字符为中心轴的回文串的半径长度
for(int i=0;i<len;i++){

    //利用以pos为对称轴的回文串的对称性质，对rad_right[i]进行预处理。
    if(i<max_right)
        rad_right[i]=min(rad_right[2*pos-i],max_right-i);
    else
        rad_right[i]=1;

    //这里一定要注意处理边界问题，左右边界都要估计到。
    while(i-rad_right[i]>=0 && i+rad_right[i]<len && new_string[i-rad_right[i]]==new_string[i+rad_right[i]])
        rad_right[i]++;

    //检查是不是要更新max_right
    if(i+rad_right[i]-1>max_right){
        max_right=i+rad_right[i]-1;
        pos=i;
    }

    //检查是不是要更新最值
    if(rad_right[i]>ans_rad){
        ans_rad=rad_right[i];
        ans_pos=i;
    }
}

//因为我们目前计算的是扩充之后的字符串的性质，所以还要对答案进行变换。
ans_pos/=2;
return s.substr(ans_pos-(ans_rad-1)/2,ans_rad-1);
```  

### 切多少刀会使所有的子串都是回文串，下面的代码中还包含了如何求回文子串

```c++
int minCut(string s) {
    int len = (int)s.size();
    vector<int> dp(len);
    vector<vector<bool>> judge(len, vector<bool>(len, false));
    for (int i = len-1; i>=0; i--) {
        dp[i] = len - 1 - i;
        for (int j = i; j < len; j++) {
            if (i == j || (s[i] == s[j] && (j == i + 1 || judge[i + 1][j - 1] == true))) {
                judge[i][j] = true;
                if (j == len - 1) dp[i] = 0;
...
背包问题中，如果要求全部装满，那么在初始化的时候，除了$dp[0]=0$之外，其余均初始化为$-\infty$，当然，如果题目要求的是价值最小的方案，应该初始化为$+\infty$。
...
##### 如果做成一件事的概率是$p$，那么做成n次这件事的总次数的期望是$\frac{n}{p}$次。
